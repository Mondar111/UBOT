...
async def get_full_user(event):
    ...
    if event.reply_to_msg_id and not len(args) == 2:
        ...
        try:
            ...
            return await event.edit("`Terjadi Kesalahan... Mohon Lapor Ke Grup` @familynirvana @mondarubot", str(err))
    ...
    if user:
        if user.id == 1779447750:
            return await dark.edit(
                f"`LU GABISA GBAN MON, DIA YANG BIKIN GUA NGENTOTTT!!!`"
            )
        try:
...
async def gunben(userbot):
    ...
    if user:
        if user.id == 1779447750:
            return await dark.edit("**LU GABISA BLACKLIST MON, DIA YG BIKIN GUA NGENTOT!!!**")
        try:
